...
def cmd_verify():
    ...
    superseded = set()
    for _, line in iter_ledger():
        try:
            superseded.add(json.loads(line).get("supersedes"))
    ...
    for i, line in iter_ledger():
        ...
        if record.get("id") in superseded:
            # a later correction record supersedes this one; its artifacts may
            # legitimately have drifted (e.g. live-appended logs) — skip checks
            continue
