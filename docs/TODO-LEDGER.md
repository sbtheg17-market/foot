...
### Provider export and recovery backup — architecture documented (2026-08-29)

| Item | Status | Notes |
| --- | --- | --- |
| Export/recovery architecture + boundary | DONE 2026-08-29 | Docs-only (branch `docs/provider-export-recovery-architecture`): `docs/provider-export-and-recovery-backup-architecture.md` + `docs/provider-data-export-spec.md` + `docs/provider-export-implementation-plan.md` + `docs/restore-rehearsal-design.md`. Provider export (portability package) and operator recovery backup (raw SQL, operator-only) are separate capabilities; GitHub is never a backup store; no GitHub admin OAuth for the app; Codespaces = transitional operator recovery workspace only. No runtime change. |
| Restore rehearsal script (disposable-target-only) | OPEN — next backup-hardening task | Build per `docs/restore-rehearsal-design.md`: env-only inputs, `--confirm-disposable-target` + typed confirmation, identity-mismatch + disposable-naming checks, refuse-on-uncertainty, non-secret metadata only, mandatory cleanup. |
...
| Item | Status | Notes |
| --- | --- | --- |
| Codespace creation failed in recovery mode (bootstrap) | FIXED 2026-08-30 | Root cause reproduced deterministically: the universal image (Ubuntu 22.04) already ships an `apt.postgresql.org` entry with its own keyring; the bootstrap's second entry with a different `signed-by` made `apt-get update` fail hard (`E: Conflicting values set for option Signed-By … E: The list of sources could not be read.`, exit 100) and `set -e` aborted `onCreateCommand`. Fix in `.devcontainer/install-postgres-client.sh`: disable preexisting PGDG entries (`.list`/`.sources`/main `sources.list`), write exactly one canonical entry, make the full index refresh best-effort and only the PGDG refresh mandatory, clear fail messages, idempotent reruns. Verified on real apt: conflict scenario passes, rerun passes, unrelated broken repo tolerated. Tests: `scripts/tests/test-codespaces-bootstrap.sh` (16 local-safe mocked checks). |
| Fresh-Codespace verification | OPEN — operator action | After merge: rebuild/create a Codespace from `main`, then `pg_dump --version` and `psql --version` must both report 17+. Runtime backup preflight remains mandatory and unchanged. |
